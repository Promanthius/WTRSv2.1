...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($first_name) || empty($last_name)) {
        ...
        if ($proceed_update) {
            if ($user['role'] === 'adviser' && $max_advisees !== null) {
                $stmt = $pdo->prepare("UPDATE users SET first_name = ?, last_name = ?, college = ?, max_advisees = ?, bio = ?, research_interests = ?, experience = ? WHERE id = ?");
                $stmt->execute([$first_name, $last_name, $college, $max_advisees, $bio, $research_interests, $experience, $user['id']]);
            } else {
                $stmt = $pdo->prepare("UPDATE users SET first_name = ?, last_name = ?, college = ?, bio = ?, research_interests = ?, experience = ? WHERE id = ?");
                $stmt->execute([$first_name, $last_name, $college, $bio, $research_interests, $experience, $user['id']]);
            }
