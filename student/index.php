...
// Refresh user object to ensure adviser_id is current
$userStmt = $pdo->prepare("SELECT adviser_id FROM users WHERE id = ?");
$userStmt->execute([$user['id']]);
$user['adviser_id'] = $userStmt->fetchColumn();
...
if ($user['adviser_id']) {
    ...
    // Check for pending release request
    $relStmt = $pdo->prepare("SELECT status FROM release_requests WHERE student_id = ? AND status = 'pending'");
    $relStmt->execute([$user['id']]);
    $pendingRelease = $relStmt->fetch();
} else {
    $pendingRelease = null;
}
...
<main class="main-content">
  ...
  <?php elseif (isset($pendingRelease) && $pendingRelease): ?>
    <div class="alert alert-info" style="background:#EFF6FF; color:#1E40AF; padding:1rem; border-radius:var(--radius-sm); margin-bottom:1.5rem; border:1px solid #DBEAFE; display: flex; align-items: center; gap: 1rem; box-shadow: var(--shadow-sm);">
      <i class="ph-bold ph-info" style="font-size: 1.5rem;"></i>
      <div>
        <strong style="display:block; margin-bottom:0.1rem;">Release Request Pending</strong>
        <span style="font-size:0.85rem;">You have requested to be released from Dr. <?= htmlspecialchars($currentAdviser['last_name']) ?>. This is currently pending approval.</span>
      </div>
    </div>
  <?php endif; ?>
      ...
      <div class="stat-card accent-teal">
        ...
        <div style="display:flex; justify-content:space-between; align-items:flex-start;">
          ...
          <?php if ($currentAdviser): ?>
            <?php if ($pendingRelease): ?>
              <span style="font-size:0.65rem; font-weight:800; color:#1E40AF; background:#DBEAFE; padding:0.2rem 0.5rem; border-radius:4px;">PENDING RELEASE</span>
            <?php else: ?>
              <a href="request_release.php" style="font-size:0.65rem; font-weight:800; color:var(--crimson); text-decoration:none; background:var(--crimson-faint); padding:0.2rem 0.5rem; border-radius:4px; transition:background 0.2s;">RELEASE</a>
            <?php endif; ?>
          <?php endif; ?>
        </div>
