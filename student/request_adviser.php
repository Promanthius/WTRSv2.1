...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($adviser_id > 0) {
        ...
        } elseif (!empty($user['adviser_id'])) {
            $error = "You already have an assigned adviser. You must be released by your current adviser before applying for a new one.";
        } else {
  ...
  <div class="form-card">
    ...
    <form method="POST">
        ...
        <div style="background:#FFFBEB; padding:1rem; border-radius:4px; margin-bottom:1.5rem; font-size:0.85rem; color:#92400E; border:1px solid #FEF3C7;">
            <strong>Note:</strong> Under the new mentorship policy, you can only have one adviser at a time. Once accepted, you cannot apply for another adviser unless your current one releases you or renounces the mentorship.
        </div>
