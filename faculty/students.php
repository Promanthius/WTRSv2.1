...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_student_id'])) {
    ...
    // Notify student
    $pdo->prepare("INSERT INTO notifications (recipient_user_id, sender_user_id, type, message) VALUES (?, ?, 'adviser_renounced', ?)")
        ->execute([$sid, $user['id'], "Prof. " . htmlspecialchars($user['last_name']) . " has released you from their mentorship. You can now request a new adviser."]);
