...
<main class="main-content">
      ...
      <?php else: ?>
        ...
        <header style="margin-bottom: 4rem;">
          <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            <div>
              <span class="type-label">Reviewing Submission</span>
              <h1 class="thesis-title-big" style="margin-bottom: 0.5rem;"><?= htmlspecialchars($thesis['title']) ?></h1>
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; font-size: 0.85rem; font-weight: 700; color: var(--text-muted);">
                <div>AUTHOR: <span style="color: var(--text-dark);"><?= strtoupper(htmlspecialchars($thesis['first_name'].' '.$thesis['last_name'])) ?></span></div>
                <div>COLLEGE: <span style="color: var(--text-dark);"><?= htmlspecialchars($thesis['college']) ?></span></div>
                <?php if ($thesis['co_authors']): ?>
                  <div style="grid-column: span 2;">CO-AUTHORS: <span style="color: var(--text-dark);"><?= strtoupper(htmlspecialchars($thesis['co_authors'])) ?></span></div>
                <?php endif; ?>
              </div>
            </div>
  ...
  <!-- PDF Preview Modal -->
  <div id="pdfPreviewModal" class="pdf-preview-modal" style="display: none; position: fixed; z-index: 9999; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.85); align-items: center; justify-content: center; backdrop-filter: blur(4px);">
    <div class="pdf-preview-content" style="background: white; border-radius: var(--radius-lg); width: 95%; height: 92vh; max-width: 1400px; display: flex; flex-direction: column; box-shadow: var(--shadow-lg); overflow: hidden;">
      <div class="pdf-preview-header" style="display: flex; justify-content: space-between; align-items: center; padding: 1.25rem 1.5rem; border-bottom: 1px solid var(--border); flex-shrink: 0; background: var(--surface);">
        <div style="display: flex; align-items: center; gap: 0.75rem;">
          <i class="ph-fill ph-file-pdf" style="color: var(--crimson); font-size: 1.5rem;"></i>
          <h3 id="modalTitle" style="margin: 0; font-size: 1.1rem; color: var(--text-dark); font-family: var(--font-serif);">Manuscript Evaluation</h3>
        </div>
        <button class="pdf-preview-close" onclick="closeModal()" style="background: var(--off-white); border: none; width: 2.5rem; height: 2.5rem; border-radius: 50%; font-size: 1.5rem; cursor: pointer; color: var(--text-muted); display: flex; align-items: center; justify-content: center;">&times;</button>
      </div>
      <div style="flex: 1; display: grid; grid-template-columns: 1fr 320px; overflow: hidden;">
        <div class="pdf-preview-viewer" style="background: #525659; overflow: hidden;">
          <iframe id="pdfFrame" src="" type="application/pdf" style="width: 100%; height: 100%; border: none;"></iframe>
        </div>
        <div class="pdf-preview-details" style="background: var(--surface); border-left: 1px solid var(--border); padding: 2rem; overflow-y: auto;">
          <h4 style="font-size: 0.7rem; font-weight: 800; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 1.5rem; border-bottom: 1px solid var(--border); padding-bottom: 0.5rem;">Submission Details</h4>
          
          <div style="margin-bottom: 1.5rem;">
            <label style="display: block; font-size: 0.65rem; font-weight: 800; color: var(--crimson); text-transform: uppercase; margin-bottom: 0.35rem;">Thesis Title</label>
            <div style="font-weight: 700; font-size: 0.95rem; line-height: 1.4;"><?= htmlspecialchars($thesis['title']) ?></div>
          </div>

          <div style="margin-bottom: 1.5rem;">
            <label style="display: block; font-size: 0.65rem; font-weight: 800; color: var(--crimson); text-transform: uppercase; margin-bottom: 0.35rem;">Primary Author</label>
            <div style="font-weight: 700; font-size: 0.9rem;"><?= htmlspecialchars($thesis['first_name'].' '.$thesis['last_name']) ?></div>
            <div style="font-size: 0.75rem; color: var(--text-muted);"><?= htmlspecialchars($thesis['college']) ?></div>
          </div>

          <?php if ($thesis['co_authors']): ?>
          <div style="margin-bottom: 1.5rem;">
            <label style="display: block; font-size: 0.65rem; font-weight: 800; color: var(--crimson); text-transform: uppercase; margin-bottom: 0.35rem;">Co-authors</label>
            <div style="font-weight: 600; font-size: 0.85rem; color: var(--text-mid);"><?= htmlspecialchars($thesis['co_authors']) ?></div>
          </div>
          <?php endif; ?>

          <div style="margin-bottom: 1.5rem;">
            <label style="display: block; font-size: 0.65rem; font-weight: 800; color: var(--crimson); text-transform: uppercase; margin-bottom: 0.35rem;">Submission Info</label>
            <div style="font-size: 0.85rem; margin-bottom: 0.25rem;">Code: <strong><?= htmlspecialchars($thesis['thesis_code']) ?></strong></div>
            <div style="font-size: 0.85rem;">Year: <strong><?= htmlspecialchars($thesis['submission_year']) ?></strong></div>
          </div>

          <div style="margin-top: 2rem; padding-top: 1.5rem; border-top: 1px dotted var(--border);">
            <p style="font-size: 0.75rem; color: var(--text-muted); line-height: 1.5; font-style: italic;">
              This preview is provided for institutional evaluation purposes. Please ensure all scholarly standards are met before approval.
            </p>
          </div>
        </div>
      </div>
