...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['release_id'])) {
    ...
    $rel_action = $_POST['rel_action'] ?? '';

    try {
        $pdo->beginTransaction();
        
        $relStmt = $pdo->prepare("SELECT student_id FROM release_requests WHERE id = ? AND adviser_id = ? AND status = 'pending' FOR UPDATE");
        $relStmt->execute([$release_id, $user['id']]);
        $relData = $relStmt->fetch();

        if ($relData) {
            if ($rel_action === 'approve') {
                $pdo->prepare("UPDATE release_requests SET status = 'approved' WHERE id = ?")->execute([$release_id]);
                $pdo->prepare("UPDATE users SET adviser_id = NULL WHERE id = ?")->execute([$relData['student_id']]);
                
                // Notify student
                $pdo->prepare("INSERT INTO notifications (recipient_user_id, sender_user_id, type, message) VALUES (?, ?, 'release_approved', ?)")
                    ->execute([$relData['student_id'], $user['id'], "Prof. " . htmlspecialchars($user['last_name']) . " has approved your release request. You can now select a new adviser."]);
                
                $success = "Release request approved. The student is no longer assigned to you.";
            } elseif ($rel_action === 'reject') {
                $pdo->prepare("UPDATE release_requests SET status = 'rejected' WHERE id = ?")->execute([$release_id]);
                
                // Notify student
                $pdo->prepare("INSERT INTO notifications (recipient_user_id, sender_user_id, type, message) VALUES (?, ?, 'release_rejected', ?)")
                    ->execute([$relData['student_id'], $user['id'], "Prof. " . htmlspecialchars($user['last_name']) . " has declined your release request."]);
                
                $success = "Release request declined.";
            }
        }
        $pdo->commit();
    } catch (Exception $e) {
...
// Fetch Student Research Portfolio and Adviser History
$studentPapers = [];
$studentAdviserHistory = [];
foreach ($requests as $req) {
    ...
    // Adviser History
    $hStmt = $pdo->prepare("
        SELECT ar.created_at as started_at, ar.updated_at as ended_at, 
               u.first_name, u.last_name
        FROM adviser_requests ar
        JOIN users u ON ar.adviser_id = u.id
        WHERE ar.student_id = ? AND ar.status = 'approved' AND ar.adviser_id != ?
        ORDER BY ar.created_at DESC
    ");
    $hStmt->execute([$req['student_id'], $user['id']]);
    $studentAdviserHistory[$req['student_id']] = $hStmt->fetchAll();
}

// Fetch pending release requests
$relReqStmt = $pdo->prepare("
    SELECT rr.*, u.first_name, u.last_name, u.college, u.email
    FROM release_requests rr
    JOIN users u ON rr.student_id = u.id
    WHERE rr.adviser_id = ? AND rr.status = 'pending'
    ORDER BY rr.created_at ASC
");
$relReqStmt->execute([$user['id']]);
$releaseRequests = $relReqStmt->fetchAll(PDO::FETCH_ASSOC);
...
<main class="main-content">
  ...
  <div style="display: grid; grid-template-columns: 2.5fr 1fr; gap: 2rem;">
    <div>
                ...
                <div class="req-card">
                    <div class="req-info">
                        ...
                        <div style="margin-top: 1rem;">
                            <button type="button" onclick='viewStudentProfile(<?= json_encode($req) ?>, <?= json_encode($studentPapers[$req['student_id']]) ?>, <?= json_encode($studentAdviserHistory[$req['student_id']]) ?>)' class="btn btn-secondary" style="font-size: 0.75rem; padding: 0.4rem 0.8rem; border-style: dashed;">
        ...
        <h3 style="font-family: var(--font-serif); font-size: 1.3rem; margin-top: 3rem; margin-bottom: 1.5rem; border-top: 1px solid var(--border); padding-top: 2rem;">Release Requests (<?= count($releaseRequests) ?>)</h3>
        
        <?php if (empty($releaseRequests)): ?>
            <div class="empty-state card-academic" style="padding: 2rem;">
                <p style="color:var(--text-muted); font-size:0.9rem;">No pending release requests.</p>
            </div>
        <?php else: ?>
            <?php foreach ($releaseRequests as $rel): ?>
                <div class="req-card" style="border-left: 3px solid var(--gold);">
                    <div class="req-info">
                        <div class="req-meta"><?= htmlspecialchars($rel['college']) ?> &bull; <?= time_ago($rel['created_at']) ?></div>
                        <h4><?= htmlspecialchars($rel['first_name'] . ' ' . $rel['last_name']) ?></h4>
                        
                        <div class="req-message">
                            <strong>Reason for Release:</strong><br>
                            "<?= nl2br(htmlspecialchars($rel['reason'])) ?>"
                        </div>
                    </div>
                    <div class="req-actions">
                        <form method="POST" style="display:contents;" class="form-confirm" data-confirm-title="Approve Release?" data-confirm-message="This student will be removed from your advisee list.">
                            <input type="hidden" name="release_id" value="<?= $rel['id'] ?>">
                            <input type="hidden" name="rel_action" value="approve">
                            <button type="submit" class="btn btn-primary" style="background: #059669; border-color: #059669;">Approve Release</button>
                        </form>

                        <form method="POST" style="display:contents;" class="form-confirm" data-confirm-title="Decline Release?" data-confirm-message="The student will remain your advisee.">
                            <input type="hidden" name="release_id" value="<?= $rel['id'] ?>">
                            <input type="hidden" name="rel_action" value="reject">
                            <button type="submit" class="btn btn-secondary" style="color:#DC2626; border-color:#FECACA;">Decline</button>
                        </form>
                    </div>
                </div>
  ...
  <div id="studentProfileModal" class="pdf-preview-modal" style="display: none; position: fixed; z-index: 10500; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); align-items: center; justify-content: center; backdrop-filter: blur(4px);">
      <div class="card-academic" style="width: 90%; max-width: 700px; max-height: 90vh; overflow-y: auto; padding: 0; border: none;">
          ...
          <div style="padding: 2.5rem;">
              ...
              <div style="margin-bottom: 2rem;">
                  <h4 style="font-weight: 800; color: var(--crimson); margin-bottom: 0.5rem; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.1em;">Advisory History</h4>
                  <div id="stuAdviserHistory" style="display: flex; flex-direction: column; gap: 0.5rem;">
...
<script>
function viewStudentProfile(student, papers, history) {
    ...
    // Inject History
    const historyList = document.getElementById('stuAdviserHistory');
    historyList.innerHTML = "";
    if (!history || history.length === 0) {
        historyList.innerHTML = "<p style='font-size:0.85rem; color:var(--text-muted); font-style:italic;'>No previous advisers recorded.</p>";
    } else {
        history.forEach(h => {
            const div = document.createElement('div');
            div.style.cssText = "padding: 0.75rem 1rem; background: var(--off-white); border-radius: var(--radius-sm); border-left: 3px solid var(--crimson); display: flex; justify-content: space-between; align-items: center;";
            const date = new Date(h.started_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
            div.innerHTML = `
                <div style="font-weight:700; font-size:0.9rem; color:var(--text-dark);">Dr. ${h.last_name}</div>
                <div style="font-size:0.7rem; color:var(--text-muted); font-weight:800; text-transform:uppercase;">Started: ${date}</div>
            `;
